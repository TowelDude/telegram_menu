...
class TelegramMenuSession:
    ...
    async def _send_start_message(self, update: Update, _: Any) -> None:  # type: ignore
        ...
        session = NavigationHandler(self.application.bot, chat, self.scheduler)
    ...
    async def _button_select_callback(self, update: Update, context: Any) -> None:  # type: ignore
    ...
    @staticmethod
    async def _msg_error_handler(update: Any, context: Any) -> None:  # type: ignore
...
class NavigationHandler:
            ...
            if button_found:
                ...
                if isinstance(message_callback, BaseMessage):
                    if message_callback.inlined:
                        msg_id = await self._send_app_message(message_callback, label)
                        if message_callback.home_after:
                            msg_id = await self.goto_home()
                    else:
                        msg_id = await self.goto_menu(message_callback)
    ...
    async def app_message_button_callback(self, callback_label: str, callback_id: str) -> None:
        ...
        if button_found.btype in [ButtonType.PICTURE, ButtonType.STICKER]:
            ...
            await self.send_poll(question=button_found.args[0], options=button_found.args[1])
        ...
        if button_found.btype == ButtonType.PICTURE:
            await self.send_photo(picture_path=action_status, notification=button_found.notification)
            await self._bot.answer_callback_query(callback_id, text="Picture sent!")
            return
        if button_found.btype == ButtonType.STICKER:
            await self.send_sticker(sticker_path=action_status, notification=button_found.notification)
            await self._bot.answer_callback_query(callback_id, text="Sticker sent!")
            return
        if button_found.btype == ButtonType.MESSAGE:
            await self.send_message(action_status, notification=button_found.notification)
